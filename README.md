...
Öppnas på `http://127.0.0.1:4317`. Dokument-API (intern testmiljö) på `http://127.0.0.1:4318` via `npm run dev:api`. Börja på `/design` för en förteckning över alla skärmar. Intern NOVUM-förhandsgranskning: `/intern`. Ingenting där är publicerat. Lekplatsutrustning har ett VVZ-Play-stickprov (lektorn VZ1-006-15 och vippgunga PHP004).
...
Avfallskatalogen har 54 serier från prislistan september 2026 (källsortering, askkopp, papperskorg). Parkbänkar, betongpollare, cykelställ och planteringskärl från prislista 2026 EUR ligger under Parkmöbler, Pollare, Cykelparkering och Plantering (fluorescerande pollare är undantagen). Listpriser i euro, rabatt, netto och leverantörens artikelnummer syns bara i admin. PETALSTEEL saknas på prislistan och är inte importerad.

STREETPARK (62 serier: parkbänkar, sittmöbler, papperskorgar, cykelställ, bord/picknick, pollare) visas med tillverkarnamn. Måttritning, 3D-perspektiv, produktblad, förankring och DWG ligger på produktsidan. Inga priser. STREETPARKs artikelnummer följer med offerten. Buss- och cykelväderskydd ingår inte. Skola och vård avvaktar.
